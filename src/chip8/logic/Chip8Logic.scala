  ...
  def step(timers: mutable.Map[String, Char]): (mutable.Map[String, Char], Boolean) = {
    val instruction = fetch()
      ...
      firstNibble match {
        ...
        case 0x3 =>
          val value = registers(secondNibble) & 0xFF
        ...
        case 0x7 =>
          registers(secondNibble) = ((registers(secondNibble) + thirdToFourthNibble) & 0xFF).toChar
        ...
        case 0x8 =>
          fourthNibble match {
            ...
            case 0x3 =>
              ...
              registers(0xF) = 0.toChar
            case 0x4 =>
              val flag: Char = if (registers(secondNibble) + registers(thirdNibble) > 255) 1 else 0
              registers(secondNibble) = ((registers(secondNibble) + registers(thirdNibble)) & 0xFF).toChar
              registers(0xF) = flag
            case 0x5 =>
            ...
            case 0x7 =>
              ...
              registers(secondNibble) = ((registers(thirdNibble) - registers(secondNibble)) & 0xFF).toChar
              registers(0xF) = flag
            case 0xE =>
              registers(secondNibble) = registers(thirdNibble)
              val flag: Char = (((registers(secondNibble).toInt & 0x80) >> 7) & 0xFF).toChar
              registers(secondNibble) = ((registers(secondNibble).toInt << 1) & 0xFF).toChar
              registers(0xF) = flag
            case 0x6 =>
              registers(secondNibble) = registers(thirdNibble)
        ...
        case 0xD =>
          val xCoordinate = (registers(secondNibble) & 0xFF) % gridDims.width
          val yCoordinate = (registers(thirdNibble) & 0xFF) % gridDims.height
